...
class Admin_model extends CI_Model
{
    ...
    public function verify_user($password,$user_id,$salt){
        $data=$this->db->select('*')
        ->get_where('efiling_users',['loginid'=>$user_id,'verified'=>'1'])
        ->result();
    ...
    public function verify_filingno($post_array){
        ...
        $rs=$this->db->where($data_array)->get('aptel_case_detail');
    ...
    public function insert_ma_ia_vakalatnama_mpapers($post_array){
        ...
        $get_full_filing_no=$this->db->select('filing_no')->where('RIGHT(filing_no,10)',$final_filing_no)->get('aptel_case_detail')->row('filing_no');
        ...
        switch (strtolower(substr($document_type,0,2))) {
            ...
            $this->db->insert('ia_detail',$ia_data_array);
            break;
        }
        ...
        if($this->db->affected_rows() ==1){
            ...
            $max_salt=$this->db->select('CAST(MAX(salt) AS INTEGER)+1 salt')->get('aptel_temp_payment')->row('salt');
            ...
            if($ia=='Yes')
                ...
                $this->db->insert('aptel_temp_payment',$payment_array);
                
                if($this->db->affected_rows() ==1){
                    $upd_salt=['salt'=>$max_salt];
                    $this->db->set($upd_salt)->where('filing_no',$get_full_filing_no)->update('aptel_case_detail',$upd_salt);
                    if($this->db->affected_rows()==1){
    ...
    public function forgot_pass($post_array){
        ...
        if(is_numeric($user_mobile)) {
            ...
            $update_pass=['password'=>$hash_pass,'is_password'=>'0'];            
            
            unset($post_array['skey_pass']);
            $updRs=$this->db->set($update_pass)->where($post_array)->update('efiling_users',$update_pass);
    ...
    public function insert_commission($post_array) {
     ...
     public function get_advocate_details($post_data){
        ...
        $rs=$this->db->select('a.adv_reg,a.adv_name,a.address,a.adv_mobile,a.email,a.adv_sex,a.state_code,a.pincode as adv_pin,a.adv_dist,b.district_name')
        ->from('master_advocate a')
        ->join('master_psdist b','a.adv_dist=b.district_code','left')
        //->where('a.status', '1')
    ...
    public function get_org_details(){
        $rs=$this->db->distinct()
        ->select('org_code as org_id,org_name')
    ...
    public function get_org_fdetails($post_data){  
        $rs=$this->db->select('a.org_code as org_id,a.address as org_address,a.state,a.pin,a.mobile_no,a.email,a.org_name as orgdisp_name,
        a.designation_short as org_desg,a.org_name as short_org_name,a.district,b.district_name')
        ->from('org_name_master a')
        ->join('master_psdist b','a.district=b.district_code','Left')
        ->where($post_data)
            ->limit(1,0)
        ->get();
        //echo $this->db->last_query();die;
    ...
    public function verifyUnique($dataArray) {
        ...
        /*switch($key) {
            ...
            case 'email':
                $rs=$this->db->select('*')->get_where('efiling_users',$dataArray);
                if($rs->num_rows() > 0) {
                    if($rs->row_array()['user_type']!='company'){
                        return 'not found';
                    }else{
                        return 'found';
                    }
   ...
   /* public function insert_newuser($post_array) {
	...
	public function insert_newuser($post_array) {
        ...
        $post_array['dinsert_date']=date('Y-m-d H:i:s');
        $post_array['dupdate_date']=date('Y-m-d H:i:s');
        if($dinsert=='success') {
            $this->db->insert('efiling_users',$post_array);
            if($this->db->affected_rows() > 0) return $make_rand_pass;
            else return false;
    ...
    public function add_remark($step,$data) {
    ...
    public function forward_appl($step,$data) {
    ...
    public function fetch_iadata($year=NULL){
        ...
        ->from('ia_detail a')
        ->join('aptel_case_detail b','b.filing_no=a.filing_no')
    ...
    function getAppRecord($key){
        $key=strtolower($this->input->post('key'));
        $this->db->select("org_code	 as org_id,org_name as short_org_name,org_name,'' as mobile_no");
        $this->db->from('org_name_master');
        $this->db->like('LOWER(org_name)',$key,'after');
        //$this->db->or_like('mobile_no', $key,'after');
        $query = $this->db->get();
      //  echo $this->db->last_query();die;
        return $query->result();
    }
