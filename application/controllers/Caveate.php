...
class Caveate extends CI_Controller {
    ...
    function cav_basicdetail(){
        $userdata=$this->session->userdata('login_success');
        $salt=$this->session->userdata('salt');
        $user_id=$userdata[0]->id;
        if($user_id){
            $data['datacomm']= $this->efiling_model->data_commission_where($salt,$user_id);
            $this->load->view('caveat/cav_basicdetail',$data);
        }
    }
    
    function cav_advocate(){
        $userdata=$this->session->userdata('login_success');
        $salt=$this->session->userdata('salt');
        $user_id=$userdata[0]->id;
        if($user_id){
            $data['datacomm']= $this->efiling_model->data_commission_where($salt,$user_id);
            $this->load->view('caveat/cav_advocate',$data);
        }
    }
    ...
    function cav_adv(){
        ...
        if($user_id){
            $data['datacomm']= $this->efiling_model->data_commission_where($salt,$user_id);
            $this->load->view('caveat/cav_adv',$data);
        }
    }
    function cav_applent(){
        $userdata=$this->session->userdata('login_success');
        $salt=$this->session->userdata('salt');
        $user_id=$userdata[0]->id;
        if($user_id){
            $data['datacomm']= $this->efiling_model->data_commission_where($salt,$user_id);
            $this->load->view('caveat/cav_applent',$data);
        }
    }
    function cav_upload_doc(){
        $userdata=$this->session->userdata('login_success');
        $salt=$this->session->userdata('salt');
        $user_id=$userdata[0]->id;
        if($user_id){
            $data['datacomm']= $this->efiling_model->data_commission_where($salt,$user_id);
            $this->load->view('caveat/cav_upload_doc',$data);
        }
    }
    ...
    function caveatorsave(){
        ...
        $this->form_validation->set_rules('pin','Enter Pin number','trim|required|min_length[6]|htmlspecialchars|regex_match[/^[0-9,]+$/]');
        if($this->form_validation->run() == FALSE){
            echo json_encode(['data'=>'error','value'=>validation_errors(),'massage'=>validation_errors(),'error'=>'1']); die;
        }
        $this->form_validation->set_rules('mob','Enter mobile number.','trim|required|min_length[10]|htmlspecialchars|regex_match[/^[0-9,]+$/]');
        if($this->form_validation->run() == FALSE){
            echo json_encode(['data'=>'error','value'=>validation_errors(),'massage'=>validation_errors(),'error'=>'1']); die;
        }
    ...
    function doc_save_nextCaveate(){
        ...
        $this->form_validation->set_rules('untak','Please Check Undertaking','trim|required|min_length[1]');
        if($this->form_validation->run() == FALSE){
            echo json_encode(['data'=>'error','value'=>'','massage'=>validation_errors(),'error'=>'1']);die;
        }
        $this->form_validation->set_rules('tabno','Valid tab no','trim|required');
        if($this->form_validation->run() == FALSE){
            echo json_encode(['data'=>'error','value'=>'','massage'=>validation_errors(),'error'=>'1']); die;
        }
        $this->form_validation->set_rules('token','Token not valid','trim|required');
        if($this->form_validation->run() == FALSE){
            echo json_encode(['data'=>'error','value'=>'','massage'=>validation_errors(),'error'=>'1']); die;
        }  
    ...
    function caveatsave(){
        ...
        $collectamountsss = $_REQUEST["collectamountsss"];
        $amountRs = $_REQUEST["amountRs"];

        $valtoal=(int)$collectamountsss+(int)$amountRs;
        if($valtoal<3025){
        ...
        if($user_id && $_REQUEST["dbankname"]!=''){
                ...
                if(!empty($st)){
                    foreach($st as $vals){
                        $data12=array(
                            ...
                            'ref_filing_no'=>$caveat_filing_no,
                ...
                $account = "insert into aptel_account_details(filing_no,dt_of_filing,fee_amount,payment_mode,branch_name,dd_no,dd_date,ia_fee)
                        ...
                        $sql1=$this->efiling_model->delete_event('temp_documents_upload','salt',$salt);
                        $sql1=$this->efiling_model->delete_event('aptel_temp_payment','salt',$salt);
    ...
    function caveatedetail($val){
        $userdata=$this->session->userdata('login_success');
        $salt=$this->session->userdata('salt');
        $user_id=$userdata[0]->id;
        if($user_id){
            $array=array('ref_filing_no'=>$val,'submit_type'=>'cave');
            $data['caveate']= $this->efiling_model->data_list_mulwhere('efile_documents_upload',$array);
            $this->load->view('caveat/caveatedetail',$data);
        }
    }
