<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Filingaction extends CI_Controller 
	{
	    ...
	    function review_petition_filing(){
	        ...
	        if($user_id){
	            $data['filedcase']= $this->efiling_model->data_list_where('aptel_case_detail','filing_no',$filingno);
	            $this->load->view("admin/review_petition_filing",$data);
	        }
	    }
		
		function cross_filing(){
	        $userdata=$this->session->userdata('login_success');
	        $this->session->unset_userdata('rpepcpsalt');
	        $filingno='';
	        $user_id=$userdata[0]->id;
	        if($user_id){
	            $data['filedcase']= $this->efiling_model->data_list_where('aptel_case_detail','filing_no',$filingno);
	            $this->load->view("admin/cross_appeal_filing",$data);
	        }
	    }
	 ...
	 function review_petition_filing1(){
  	...
  	function edit_contempt_petition_filing1(){
		...
		{
	        $q = $this->input->post('q');
			$this->db->select("org.address,org.org_name,org.mobile_no,org.email,org.phone_no,org.pin,org.fax,org.state,org.district,org.designation_short,org.zone");
			$this->db->select("st.state_name,dis.district_name");
			$this->db->from("org_name_master as org");
			$this->db->join("master_psstatus as st","st.state_code=org.state","left");
			$this->db->join("master_psdist as dis","dis.district_code=org.district","left");
			$this->db->where(['org.org_code'=>$q]);
			$query =$this->db->get();
			$row=$query->row();
			$add = $row->address;
			...
			$orgdesg = $row->designation_short;
			$statename = $row->state_name;
			$distname = $row->district_name;
			$users_arr[] = array("orgid"=>$q,"org_name"=>$org_name,"address" => $add, "mob" => $mob, "mail" => $mail, "ph" => $ph, "pin" => $pin, "fax" => $fax, "stcode" => $stateCode, "stname" => $statename,
				"zone"=>$row->zone,"dcode" => $distcode, "dname" => $distname, "desg" => $orgdesg,$this->security->get_csrf_token_name()=>$this->security->get_csrf_hash());
...
'ip'=>$ipaddress,'update_type'=>$update_type);
	    ...
	    function iaprint_rp_cp_ep(){
	        ...
	        $row =$this->efiling_model->data_list_where('aptel_case_detail','filing_no',$filing_no);   
	        $dt_of_filing= $row[0]->dt_of_filing; 
	        $params['data'] = "$val , Filing Date $dt_of_filing  $url";
	        $params['level'] = 'H';
	        $params['size'] = 10;
			$data['image']= 'dummy_qr.png';
			//  $path= FCPATH.'qrcodeimg/'.$filing_no.'.png';
			if($this->config->item('isGdLib')==true):
	    ...
	    function check_valaltanama(){
	        ...
	        if ($flags == 'P') {
	            $stha20_aptel_case_detail = $this->efiling_model->data_list_where('aptel_case_detail','filing_no',$filing_no);
    ...
    function ia_action(){
      ...
      function documentUpload(){
            ...
            function iaprint($filing_no,$iano,$iayear){
                ...
                $user_id=$userdata[0]->id;
                if($user_id){
                    $ias= base64_decode($iano);
                    $data['filing_no']=$filing_no;
                    $data['iano']=base64_decode($iano);
                    $data['year']=$iayear;
  ...
  function qrcode($path){			
		$path=UPLOADPATH."/qrcodeci/".$path;
            ...
            function filingPrintSlip(){
                ...
                $sql22 = $this->efiling_model->data_list_where('aptel_case_detail','filing_no',$filing_no);
                $user = $this->efiling_model->data_list_where('efiling_users','id',(int)$sql22[0]->user_id);
                ...
                $val= "DIARY NO/".displayDiaryNo($filing_no);
                $url= base_url();
				...
				//if($this->config->item('isGdLib')==true):
				$this->load->library('ciqrcode');
					$params['savename'] = UPLOADPATH.'/qrcodeci/'.$filing_no.'.png';
					$path = UPLOADPATH.'/qrcodeci';
					if (!file_exists($path)) {
						mkdir($path, 0777, true);
					}
					if(!file_exists($path.'/'.$filing_no.'.png')) {
					$this->ciqrcode->generate($params);
				}
					$data['image']= $filing_no.'.png';
				//endif;
            ...
            function payslip($id){
                ...
                $user_id=$userdata[0]->id;
                $this->load->library('ciqrcode');
                $sql22 = $this->efiling_model->data_list_where('aptel_case_detail','filing_no',$id);
            ...
            function getQrfilingIA($filing_no,$ias){
                ...
                $dtoffiling='';
				$ias=rtrim($ias,',');
                $query=$this->db->query("select * from ia_detail where ia_no in($ias) and filing_no='$filing_no'");
                $iasd=$query->result();
                if(!empty($iasd) && is_array($iasd)){
                    $dtoffiling=$iasd[0]->entry_date;
                    $dtoffiling=date('d/m/Y',strtotime($dtoffiling));
                }
                ...
                $val= "DFR/$filing_No/$filingYear";
                $url= "https://cis.aptel.gov.in/";
                $params['data'] = "IA NO $ias of $yearc  in  $val  Filing Date $dtoffiling filed by $name , $pet_email ,Mobile No. $pet_mobile  $url";
                $params['level'] = 'H';
                $params['size'] = 10;
                $params['savename'] = FCPATH.'qrcodeci/'.$filing_no.'.png';
                $path = './qrcodeci/';
...
/*             
            ...
            function maprint($filing_no,$docfilingno,$docType){
                $userdata=$this->session->userdata('login_success');
                $user_id=$userdata[0]->id;
                if($user_id){
                    $qrcode=$this->getQrfilingDoc($filing_no,$docfilingno,$docType);
                    $data['doc_filing']=$docfilingno;
            ...
            function getQrfilingDoc($filing_no,$docfilingno,$doctype){
               ...
               // error_reporting(E_ALL);
                ...
                $name=$user[0]->fname.' '.$user[0]->lname;
                if($doctype=='ma'){
                    $docd = $this->db->query("select * from ma_detail where  filing_no='$filing_no' AND  ma_filing_no='$docfilingno' AND doc_type='$doctype'");
                    $docdv = $docd->result();
                    foreach ($docdv as $val) {
                        ...
                        $dt_of_filing= $val->dt_of_filing;
                        $doc=$val->doc;
                    }
                }
                
                if($doctype=='va'){
                    $docd = $this->db->query("select * from vakalatnama_detail where  filing_no='$filing_no' AND  vakalatnama_no='$docfilingno'");
                    $docdv = $docd->result();
                    foreach ($docdv as $val) {
                ...
                $val= "DFR/$filing_No/$filingYear";
                $url= "https://cis.aptel.gov.in/";
                $params['data'] = "MF No. $docfilingno of $yearc  in  $val  Filing Date $dt_of_filing filed by $name , $pet_email ,Mobile No. $pet_mobile  $url";
                $params['level'] = 'H';
                $params['size'] = 10;
                $params['savename'] = FCPATH.'qrcodeci/'.$docfilingno.$yearc.'.png';
                $path = './qrcodeci/';
                if (!file_exists($path)) {
                    mkdir($path, 0777, true);
                }
                $this->ciqrcode->generate($params);
                return  $docfilingno.$yearc.'.png';
            }
