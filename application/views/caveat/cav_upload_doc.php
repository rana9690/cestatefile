...
<div id="rightbar"> 
<div id="progreess" style="display:none;"><img src="<?php echo base_url(); ?>asset/images/preview.gif" style=" width: 160px; height: 160px;"></div>
	...
	<div class="row">
      ...
      <?php 
            ...
            $st=$this->efiling_model->data_list_where('aptel_temp_appellant','salt', $salt);
        	    ...
        	    <input type="hidden" name="valtype" id="valtype" value="cave">
                <div class="col-md-12">
                  <h3 class="text-info">All Files should be pdf format only <sup class="text-danger">*</sup></h3>
				</div>
                <div class="col-md-12">
                  <div class="row rdoc">
                    <div class="col-md-2"> Document Filed By <sup class="text-danger">*</sup> </div>
                    <div class="col-md-4">
                      <select id="party_type" class="form-control">
                        <option value="">Document filed by</option>
                        <option value="Caveator" selected>Caveator</option>
                      </select>
                    </div>
                    <div class="col-md-2">
                 ...
                 <?php
                    ...
                    $doc=$this->efiling_model->data_list_where('master_document_efile','doctype', 'caveate');
                    ?>
                    <div class="col-md-4">
	          ...
	          // $('#loading_modal').modal();
	            if(file != undefined && type == "application/pdf") { 
	                ...
	                if(dots > 1){  
	                    ...
	                    $.ajax({
	                        type:'post',
	                        url: base_url+'required_cav/'+token_hash,
	                        data: formdata,
	                        processData: false,
	                        contentType: false,
	                        dataType: 'JSON',
...
$('#documentUpload').submit(function(e){
  ...
  if(untak=='0'){
	  alert("Please Check undertaking !");
	  return false;
  }
  $.ajax({
    type: "POST",
    url: base_url+"doc_save_nextCaveate",
    data: {salt:salt, token:token, tabno:tabno, untak:untak},
    dataType: 'json',
    success: function (resp) {
      if(resp.data=='success') {
       setTimeout(function(){
            window.location.href = base_url+'cav_checklist';
         }, 250); 
      }
