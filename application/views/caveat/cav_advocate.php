        ...
        <div class="row">
            <div class="card" style="width: 100%; padding: 0px 12px;border-top: 0px;margin-top: 0px; border-top-right-radius: 0px; border-top-left-radius: 0px;">
                ...
                <div class="row alert alert-warning" style="padding: 8px 0px 0px; font-weight: 600;font-size: 18px; margin: -20px auto 12px; max-width: 100%;">
                    ...
                    <div class="row">
                    <?php 
                    $app= $this->efiling_model->data_list_where('temp_caveat','salt',$salt);
...
function addCouncel(str) {
    ...
    if(advtype=='2'){
     ...
     $.ajax({
        type: "POST",
        url: base_url+'addCouncelCaveate',
        data: dataa,
        cache: false,
	...
	$.ajax({
        ...
        success: function (resp) {
        	...
        	if(resp.data=='success') {
        		setTimeout(function(){
                    window.location.href = base_url+'cav_upload_doc';
                 }, 250);
        	    document.getElementById("loading_modal").style.display = 'none';
        	    $('#step_11').removeClass('btn-danger btn-warning btn-info').addClass('btn-success');
        	    $('#step_4').removeClass('btn-danger btn-warning btn-info').addClass('btn-warning');
			}
...
function deletePartyadv(e) {
	...
	$.ajax({
        type: "POST",
        url: base_url+'deleteAdvocateCaveate',
        data: dataa,
        cache: false,
		beforeSend: function(){
