...
<form action="#" id="frmCounsel" autocomplete="off">    
    <input type="hidden" id="saltNo" name="saltNo" value="<?php echo $salt; ?>">
                ...
                <div class="row alert alert-warning" style="">
                    ...
                    $pet_council_adv=isset($app[0]->pet_adv)?$app[0]->pet_adv:''; 
                    ...
                    ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group required" id="div-add">
                            	...
                            	<input type="text" name="advname" value="" id=advname  class="form-control" onkeypress="serchrecordvaladv(this.value);">
                    ...
                    <div class="row">
                        ...
                        <br>
                        <input type="button" name="" class="btn btn-primary btn-cntr" onclick="addCouncel()" value="Add">
                    </div>
                </fieldset>
    <div class="row m-0">
           ...
           if($counseladd!=''){
               ...
               if($main==''){
                   if (is_numeric($vals[0]->pet_adv)) {
                       ...
                       foreach($orgname as $vals1){
                           $adv_name=$vals1->fname.' '.$vals1->lname;
                           $adv_reg=$vals1->id_number.' <span style="color:red">'.$vals1->idptype.'</span>';
                           $adv_mobile=$vals1->mobilenumber;
                           $email=$vals1->email;
                           $address=$vals1->address;
                           $pin_code=$vals1->pincode;
                           if($vals1->state!=''){
                               $st2 = $this->efiling_model->data_list_where('master_psstatus','state_code',$vals1->state);
                               $statename= $st2[0]->state_name;
                           }
                           if($vals1->district!=''){
                               $st1 = $this->efiling_model->data_list_where('master_psdist','district_code',$vals1->district);
                               $ddistrictname= $st1[0]->district_name;
                           }
                       }
                   }
...
function deletePartyadv(e) {
	...
	$.ajax({
        type: "POST",
        url: base_url+'deleteAdvocaterefile',
        data: dataa,
        cache: false,
		beforeSend: function(){
...
function editPartyAdv(id,type,advType) {
    ...
    $.ajax({
        type: "POST",
        url: base_url+'getAdvDetailEditrefile',
        data: dataa,
        cache: false,
        success: function (petSection) {
        	 var data1 = petSection;
...
function serchrecordvaladv(val){
	$.ajax({
		type: 'post',
		url: base_url+'getAdv',
		data: {key:val,csrf_token:$("#csrf_token").val(),saltNo:$("#saltNo").val()},
		dataType: 'html',
		cache: false,
		beforeSend: function(){
		 	//$('#loading_modal').modal();
