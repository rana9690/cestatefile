...
if($party=='appleant'){
    ...
    if($vals[0]->pet_name!=''){
        ...
        <tr style="color:green">
            ...
            <td><input type="button" name="nextsubmit" id="nextsubmit" value="Edit" class="btn1 btn btn-xs btn-warning" data-toggle="modal" data-target="#exampleModal"  onclick="editParty('0','<?php echo $type; ?>','<?php echo $type; ?>')"></td>
            <td></td>
        </tr>
      <?php 
    }
    $st=$this->efiling_model->delete_event('aptel_temp_additional_party', 'id', $id);
    $feesd=$this->efiling_model->data_list_where('aptel_temp_additional_party','salt',$salt);
    $i=2;
    foreach ($feesd as $row){
        ...
        <tr>
            ...
            <td><input type="button" name="nextsubmit" id="nextsubmit" value="Edit" class="btn1 btn btn-xs btn-warning" data-toggle="modal" data-target="#exampleModal"  onclick="editParty('<?php echo $row->id; ?>','<?php echo $appleant; ?>','<?php echo $type; ?>')"></td>
            <td><input type="button" name="nextsubmit" id="nextsubmit" value="Delete" class="btn1 btn btn-xs btn-danger" onclick="deleteParty('<?php echo $row->id; ?>','<?php echo $appleant; ?>')"></td>
        </tr>
    <?php
    $i++;
    }
...
if($party=='res'){
    ...
    $vals=$this->efiling_model->data_list_where('aptel_temp_appellant','salt',$salt);
     if($vals[0]->resname!=''){
      ...
      <?php } 
      $st=$this->efiling_model->delete_event('aptel_temp_additional_res', 'id', $id);
      $appleant="res";
      $type="add";
      $feesd=$this->efiling_model->data_list_where('aptel_temp_additional_res','salt',$salt);
      $i=2;
      foreach ($feesd as $row){
