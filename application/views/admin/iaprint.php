...
<a target="_blank" href=""> <font color="red" size="1"><img src="<?php echo base_url();?>asset/images/print.gif"  height="22" width="62px" class="no-print" border='0'/></font></a>
</div>
<?php
...
$filedby='';
    for($i=0;$i<=sizeof($iaNo1)-1;$i++){
		if($iaNo1[0]!=""){
    		$sqlia="select * from ia_detail where ia_no='$iaNo1[$i]' and SUBSTR(ia_filing_no,12,4)='$iaYear'";
...
<div style="position: relative;">
    ...
    <table border="1" cellpadding="3" style="width:100%; border-collapse: collapse;">
	   ...
	   <?php 
	    ...
	    $sqlia="select * from ia_detail where ia_no in ($ia_requestt) and filing_no = '".$filingNo."' and status = 'P' ";
	    $query=$this->db->query($sqlia);
	    $data = $query->result();
	    foreach ($data as $row){
		   ...
		   $filedbyName = '';
			...
			$stQ = "select nature_name from moster_ia_nature where nature_code= '$ianature'";
			$query=$this->db->query($stQ);
			$data = $query->result();
    ...
    <table border="0" style="width:100%;">
        <tr>
          ...
          <?php
                 ...
                 $stnameaccount_details = "select * from aptel_account_details where filing_no = '$filingNo'
