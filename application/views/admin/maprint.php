...
$mano = $doc_filing;
if($type=='ma'){
$docd = $this->db->query("select * from ma_detail where  filing_no='$filingNo' AND  ma_filing_no='$mano' AND doc_type='$type'");
$docdv = $docd->result();
    ...
    <div style="overflow: hidden;">
        ...
        <div style="float: right; width: 50%; text-align: right;">
            <p>DATE OF FILING : <?php echo $dateprint; ?></p>
             <p> <img src="<?php echo base_url(); ?>qrcodeci/<?php echo $image; ?>" height="100px"></img></p>
        </div>
    </div>
    ...
    <table border="0" style="width:100%;">
        <tr>
            <td style="width:100%;" valign="top">
                <?php
                if ($type == 'va') {
                    $stname =$this->db->query("select ms.adv_name from master_advocate as ms INNER JOIN 
                    vakalatnama_detail as vk on ms.adv_code=vk.adv_code  where vk.filing_no='$filingNo' and vk.vakalatnama_no='$mano'");
                    $stname=$stname->result();
                    echo "<p><b>Filed By:-  ";
                    echo $advname = isset($stname[0]->adv_name)?$stname[0]->adv_name:'';
                    echo '</b></p>';
                }
                if ($type != 'doc') {
                 $date_fing = date('Y-m-d');
                 $stnameaccount_details = $this->db->query("select * from aptel_account_details where filing_no = '$filingNo' 
                        and  dt_of_filing = '$date_fing' AND reff_filing_no='$mano' ORDER BY id ASC");
                 $stnameaccount_details=$stnameaccount_details->result();
                 $total_feee = 0;
                 if(!empty($stnameaccount_details)) { 
                     foreach ($stnameaccount_details as $valuecs) {
                         $total_feee+=$valuecs->amount;
                    }
                 }?>
               <p><b>Amount Received :- RS. <?php echo $total_feee; ?></b></p>
               <?php }  if ($matter != '') { ?>
               <p><b>Matter :  <?php echo $matter; ?></b></p>
                <?php } ?>
            </td>
