	...
	<div class="row">
      ...
      <?php 
            ...
            $st=$this->efiling_model->data_list_where('aptel_temp_appellant','salt', $salt);
        	    ...
        	    <input type="hidden" name="valtype" id="valtype" value="cert">
                        <div class="col-md-12">
                          ...
                          <div class="row rdoc">
                            ...
                            $doc=$this->efiling_model->data_list_where('master_document_efile','doctype', 'cert');
                            ?>
                            <div class="col-md-4">
	          ...
	          // $('#loading_modal').modal();
	            if(file != undefined && type == "application/pdf") { 
	                ...
	                if(dots > 1){  
	                    ...
	                    $.ajax({
	                        type:'post',
	                        url: base_url+'required_cert/'+token_hash,
	                        data: formdata,
	                        processData: false,
	                        contentType: false,
	                        dataType: 'JSON',
...
$('#documentUpload').submit(function(e){
  ...
  if(untak==0){
	  alert("Please Check Undertaing !");
	  return false;
  }
  
  $.ajax({
    type: "POST",
    url: base_url+"doc_save_nextcert",
    data: {salt:salt, token:token, tabno:tabno, untak:untak},
    dataType: 'json',
    success: function (resp) {
      if(resp.data=='success') {
       setTimeout(function(){
            window.location.href = base_url+'certpf';
         }, 250); 
      }
            ...
            success: function(rtn){
              if(rtn.error=='0'){
                ...
                $.each(rtn.data, function(i,item) {
                  ...
                  valumenovc='';
                  if(valumeno!=''){
                     var valumenovc = '<span style="color:red">'+item.valumeno+'</span>';
                  }
