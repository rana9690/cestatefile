    ...
    if (act_id.length > 0) {
        $.ajax({
            ...
            success: function (response) {
                ...
                for (i in List ) {
                    div_data+='<option value="' + i + '">' + List[i] + '</option>';
                }

                $("#petSection").html(div_data);
            }
        });
    }
	}
...
/*use in applicant in oa file*/
$(".applicant-border-style").hide();
$(".selectOrg").click(function() {
    console.log(this.value);
    if(this.value==2) {
        $("#orgmaster").html('<div class="form-group required" id="orgmaster">' +
            '<label>Assess Name<span class="text-danger">*</span></label>' +
            '<input type="hidden" name="orgid" value="" id="orgid" class="txt">' +
            '<input type="text" name="petName" value="" id=petName  class="form-control">' +
            '<ul class="autosuggest" id="regnum_autofill"></ul>' +
            '<div class="col-md-9" id="recordadv"></div></div>');
        $(".applicant-border-style").show(300);
    } else {
        $("#orgmaster").html('<div class="form-group required" id="orgmaster">' +
            '<label>Department Name (Jurisdictional Commissioner)<span class="text-danger">*</span></label>' +
            '<input type="hidden" name="orgid" value="" id="orgid" class="txt">' +
            '<input type="text" name="petName" value="" id=petName  class="form-control" onkeypress="serchrecordvalapp(this.value);">' +
            '<ul class="autosuggest" id="regnum_autofill"></ul>' +
            '<div class="col-md-9" id="recordadv"></div></div>');
        $(".applicant-border-style").hide(200);
    }
});
