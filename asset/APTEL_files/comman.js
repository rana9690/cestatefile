...
$('form ,select, input,textarea,email,number').attr('autocomplete','off');
}else if(fnBrowserDetect()=='chrome'){
$('form ,select, input,textarea,email,number').attr('autocomplete','off');
}
else{
$('form ,select, input,textarea,email,number').attr('autocomplete','off');
	...
	$('#userLogin').submit(function(e){
		...
		if(loginid != '' && pass != '' && captcha != ''){
			...
			$.ajax({
					...
					data: dataa,//{eid: loginid, pass: pass, skey: captcha},
					beforeSend: function(){		
						$('#pwd').val(pass);
					...
					success: function(respons){
						if(respons.login_type=='success') {						
							...
							$('#captcha_data').html(respons.captcha);
							$('.responseMsg').removeClass('hide');
							$('#responseMsg').text(respons.message);
			  	...
			  	$.ajax({
			  		...
			  		success: function(retr) {
			  			if(otp_type== 'mobilenumber' && retr.data=='success') { 
			  				$('.motp_msg').html('<i class="fa fa-check-circle"></i>&nbsp;OTP has been send.<input type="number" placeholder="Enter OTP" value="" id="mobileOtp" autocomplete="__away" class="form-control">');
			  				$.alert("OTP has been send on your enterd mobile number.");
			  				$('.mobilecaptcha').removeClass('hide');
			  				$('#captcha_data_pass_newmobile').html(retr.returntoken['image']);
			  				$('#mobileOtp').focus();
			  			}
			  			else if(otp_type == 'email' && retr.data=='success') {
			  				$('.eotp_msg').html('<i class="fa fa-check-circle"></i>&nbsp;OTP has been send.<input type="number" placeholder="Enter OTP" value="" id="emailOtp" autocomplete="__away" class="form-control">');
			  				$.alert("OTP has been send on your enterd email id.");		  					  					
		  					$('#idptype').attr('disabled',false), $('#emailOtp').focus();
		  					$('#captcha_data_pass_newemail').html(retr.returntoken['image']);
		  					$('.emailcaptcha').removeClass('hide');
			  			}
	...
	$('#regFormFinal').submit(function(e){
		e.preventDefault();
		   ...
		   var iv = 'a1a2a3a4a5a6a7a8b1b2b3b4b5b6b7b8';
				...
				var encrypted = CryptoJS.AES.encrypt(idnumverval, keyBytes, {
					iv: ivBytes,
		   ...
		   data['idnumverval']=encrypted,
		   data['id_reff']=id_reff;
		   data['token']=token;
		   data['token_hash']=token_hash;
		    $.ajax({
				type: 'post',
				url:  base_url+'regFormFinal/'+token_hash,
				data: data,
				dataType: 'json',
				beforeSend: function(){
					$('#regButton').prop('disabled',true).val("Under proccess....");
		...
		$.ajax({
                ...
                success: function(retn){
                	if(retn.data=='success'){
                		...
                		$.alert({
							title: '<i class="fa fa-check-circle"></i>&nbsp;</b>Congrates</b>',
							content: "<p class='text-success'>Your password has been send on registred mobile number.\r\nKindly check in mobile message box.</p>",
							animationSpeed: 2000
						});
						$(this).trigger("reset");
						setTimeout(function(){ 
...
function verify_otp(click_idss) {	
	var captchav=''; 
		if(click_idss=='1'){
			var click_id='mobileOtp';
			captchav = $("#mobileOtpcaptcha").val();
		}
		if(click_idss=='2'){
			var click_id='emailOtp';
			captchav = $("#emailOtpcaptcha").val();
		}
...
$('#mobileOtpcaptcharef').click(function(e){
	  	e.preventDefault();
	  	var url='';
	  	url=window.location.href;
	  	$.ajax({
	  		type: 'post',
	  		url: url+'crefresh_valmobile',
	  		type: 'html',
	  		success: function(respons){
	  			$('#captcha_data_pass_newmobile').html(respons);
	  		},
	  		error: function(){
	  			$.alert('Server error, try later!');
	  		}
...
$('#emailOtpcaptcharef').click(function(e){
	  	e.preventDefault();
	  	var url='';
	  	url=window.location.href;
	  	$.ajax({
	  		type: 'post',
	  		url: url+'crefresh_valemail',
	  		type: 'html',
	  		success: function(respons){
	  			$('#captcha_data_pass_newemail').html(respons);
	  		},
	  		error: function(){
	  			$.alert('Server error, try later!');
	  		}
